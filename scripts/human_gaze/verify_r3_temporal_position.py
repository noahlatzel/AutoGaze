...
def verify_run(root, arm, seed, max_ratio):
    ...
    config = OmegaConf.to_container(OmegaConf.load(directory / "config.yaml"), resolve=True)
    expected_mode = "none" if arm == "control" else "normalized_sinusoidal_scalar_gate"
    assert config["model"]["gaze_model_config"]["temporal_position_encoding"] == expected_mode
    ...
    if arm == "position":
        gate = float(endpoint["temporal_position_gate"])
        ratio = float(endpoint["temporal_signal_to_feature_rms"])
...
def main():
    ...
    for seed in BASE_SEEDS:
        control_config, endpoints["control"][str(seed)] = verify_run(
            args.run_root, "control", seed, args.max_signal_ratio
        )
        position_config, endpoints["position"][str(seed)] = verify_run(
            args.run_root, "position", seed, args.max_signal_ratio
        )
        differences = set(nested_differences(control_config, position_config))
        if differences != allowed_differences:
            raise AssertionError(f"Unexpected paired config differences for {seed}: {sorted(differences)}")
    report = {
        "schema_version": 1,
        "status": "verified",
        "job_id": JOB_ID,
